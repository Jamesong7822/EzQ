...
project.
This server class will be runned on the cloud /
local platform with sufficient computing power
for dectection of humans.
...
class EzQServer():
    ...
    def __init__(self):
        print ("Hello, I am EZQ, your one stop helper in queue-order optimizations!")
        ...
        # Resize Window
        cv2.resizeWindow("HI", self.screen_w, self.screen_h)
    ...
    def draw_rects(self, image, rects):
        ...
        """
        x, y, w, h = rects
        img = cv2.rectangle(image, (int(x-w/2),int(y-h/2)),(int(x+w/2),int(y+h/2)), (0,255,0), 2)
